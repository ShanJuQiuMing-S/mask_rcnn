...
class ProposalLayer(Layer):
    ...
    def call(self, inputs):
        ...
        # 1 获取置信度 代表这个先验框内部是否有物体[batch, num_rois, 1]
        scores = inputs[0][:, :, 1]

        # 2 获取bias 代表这个先验框的调整参数[batch, num_rois, 4]
        deltas = inputs[1]

        # 2.1 reshape [0.1 0.1 0.2 0.2]，改变数量级
...
def parse_image_meta_graph(meta):
    ...
    """
    image_id = meta[:, 0]          # id
    original_image_shape = meta[:, 1:4]  # 原始图片的shape
    image_shape = meta[:, 4:7]    # 调整后图片的shape
...
class PyramidROIAlign(Layer):
    ...
    def call(self, inputs):
        ...
        # 1.2 mage_meta包含了一些必要的图片信息
        image_meta = inputs[1]

        # 1.3 取出所有的特征层[batch, height, width, channels]
...
class DetectionLayer(Layer):
    ...
    def call(self, inputs):
        ...
        # 2 图片分割 找到window的小数形式
        m = parse_image_meta_graph(image_meta)
        image_shape = m['image_shape'][0]  # 输入到maskRcnn的图片大小
        window = norm_boxes_graph(m['window'], image_shape[:2]) # 在边缘加灰条，window是非灰条部分

        # 3 细化分类建议并过滤重叠部分并返回最终结果探测 Run detection refinement graph on each item in the batch
