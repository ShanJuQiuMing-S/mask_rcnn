...
#   RPN模型
#------------------------------------#
def build_rpn_model(anchors_per_location, depth): # depth=featureMap的通道数
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    x = PyramidROIAlign([pool_size, pool_size],
                        name="roi_align_classifier")([rois, image_meta] + feature_maps) # x是抠图
...
def build_fpn_mask_graph(rois, feature_maps, image_meta,
    ...
    x = PyramidROIAlign([pool_size, pool_size],  # pool_size=14  14*14*256
...
def get_predict_model(config):
    ...
    # 最终预测框
    detections = DetectionLayer(config, name="mrcnn_detection")(  # RPN层的rpn_rois建议框, mrcnn_class, mrcnn_bbox, input_image_meta
